...
bitcoin.SelectParams("mainnet") # Select the network (testnet or mainnet)
my_private_key = CBitcoinSecret('L1pHjuH5Hf3vXy8iYxs9v3u838JEAV4pe3WxfEMnnJuoSz9JCcAd') # Private key in WIF format XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
def make_coinbase_transaction(reward_amount, txin_address, txin_index, output_script, coinbase_bytes_data):
...
def create_initial_header(prev_block_hash, merkle_root, mine_target:int, version):
    ...
    return version_bytes + prev_block_hash_bytes + merkle_root_bytes + timestamp_bytes + target_bytes
...
def print_mining_info(coinbase_tx:CMutableTransaction, prev_block_number, prev_block_hash, block_reward, coinbase_data, coinbase_hex_data, block_hash, header, block_body):
...
if __name__ == '__main__':
    prev_block_number = 9334
    prev_block_hash = '00000000261b4765edf334510ef4e167cbaf58406af6281891b4d57595c10fc9'
    block_reward = 3.125
    coinbase_data = '810199334AlirezaArbabi'
    ...
    coinbase_tx = make_coinbase_transaction(block_reward, coinbase_txin, coinbase_utxo_index, output_script, coinbase_scriptSig)
    merkle_root = calculate_merkle_root(coinbase_tx)
    target = calculate_mine_target(16)
    initial_header = create_initial_header(prev_block_hash, merkle_root, target, version=1)
    header , block_hash = mine_next_block(initial_header, target.to_bytes(32, 'big'))
    block_body = header + coinbase_tx.serialize()
    print_mining_info(coinbase_tx, prev_block_number, prev_block_hash, block_reward, coinbase_data, coinbase_hex_data, block_hash, header, block_body)
